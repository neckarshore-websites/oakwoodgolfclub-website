...
| 22 | **Formular-Autoresponder (Phase 1)** — Alle 3 Formulare (Kontakt, Signup, Renewal) sollen eine Bestätigungs-E-Mail an den Absender schicken, zusätzlich zur Notification an `info@oakwoodgolfclub.de`. Template pro Formular. UX-wichtig (Absender weiß, dass's angekommen ist), Trust-Signal, DSGVO-Transparenz. Technisch: zweiter SMTP-Send-Call per Formular-Submission, Reply-To bleibt `info@`. | P1 | S | Linus | — | Offen | User-Vertrauen + professioneller Eindruck vor Launch |
...
| ~~D15~~ | B9 Content-Migration MVP — `scripts/migrate-posts.py` (markdownify + Avada-Shortcode-Stripper + http→https). 20 WP-Posts → `content/blog/*.md` mit Frontmatter. Handicap-Posts (2) auto-draft. 18 publish-Posts + 4 Kategorien live. | (pending this commit) | 2026-04-17 |
