...
| B14-G6 | ~~Vercel-Hobby-Plan ohne GitHub-Auto-Deploy (Backlog #15). Nach Cutover sind Deploys weiter manuell via `npx vercel deploy --prod`. Bedeutet: nach B14 passiert **nichts** automatisch, wenn man `git push` macht.~~ **⚠️ SUPERSEDED 2026-06-12 — diese Annahme ist jetzt INVERTIERT.** Seit dem 2026-06-11 Org-Split (Repo public unter `neckarshore-websites`) ist Auto-Deploy-on-push LIVE: ein `git push`/Merge auf `main` deployt automatisch zu Production und erreicht ~300 zahlende Mitglieder. Backlog #15 CLOSED. | ~~Item #15 bleibt offen~~ → **Jeder Merge auf `main` = Production-Release.** Bei Member-Flow-Änderungen den Test-Signup/Renewal/Handicap-Round-Trip nach dem Deploy fahren. |
